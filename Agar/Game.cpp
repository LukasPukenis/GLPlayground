...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode) {
	game_instance->keyCallback(key, action);	
}

void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
	game_instance->mouseCallback(xpos, ypos);
}


// OpenGL >= 4.1 error callback, saves you from constant checking after each step
...
void Game::setup(const std::string & mapName, GLuint width, GLuint height) {
	WIDTH = width;
	HEIGHT = height;
	game_instance = this;
